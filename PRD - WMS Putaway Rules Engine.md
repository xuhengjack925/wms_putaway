...
Yes—this is expected behavior. When items 1-12 fill BIN-A-001 and item #13 needs placement, the engine re-evaluates with `lastPutLocation = BIN-A-001`. If the preference is "Fill Most Full" without proximity in orderBy, the engine selects BIN-B-045 (50m away, 95% full) over BIN-A-002 (adjacent, 60% full). The stower walks across the warehouse because space efficiency is the configured priority.

To optimize travel for partial fills, configure: `OrderBy: [Proximity to Last Location, Fill Most Full]`. This makes proximity primary, selecting BIN-A-002 despite lower utilization. The system requires explicit configuration—users encode intent, not implicit defaults.
...
Detection is reactive by design. When Merchant X onboards without specific rules, generic preferences apply—ABC-A items flow to Golden Zone, ABC-C to Reserve. Items scatter by velocity classification across 12 aisles until an operations manager notices the dispersion two weeks later.

The system cannot proactively detect "missing" rules because it cannot know merchant intent. Small DTC brands may share generic space while large B2B clients require isolated zones. If Merchant X triggers 50 overrides with reason "BETTER_ORGANIZATION," the defect log reveals the pattern. SMEs then create: `merchant_id = 'Merchant_X' → zone_id IN ['Zone_5']`.

Mitigation: Merchant onboarding checklists should mandate rule configuration before first receipt. Defect monitoring alerts when new merchant_id values appear in override logs. This is configuration governance, not system automation.
...
Yes—this is a known Phase 1 limitation. A 12x20x20-inch item gets recommended to BIN-C-045 (correct reserve zone, 80% utilization) but the middle shelf has only 15-inch clearance. The stower overrides to BIN-C-012 (lower shelf, 24-inch clearance) with reason "BIN_TOO_SMALL." The defect log shows a rule-compliant recommendation overridden, masking that the root cause is missing dimension validation, not misconfiguration.

Capacity validation requires complete dimension data for all products and locations (often unavailable), plus complex calculations for orientation, mixed-SKU stacking, and weight limits. Phase 1 defers this complexity.

Mitigation: Define granular storage types—`SHELF_LOWER` (0-18"), `SHELF_MIDDLE` (18-36"), `SHELF_UPPER` (36-60"). Create constraints: `height > 18" → storage_type IN [SHELF_LOWER, FLOOR]`. Monitor `BIN_TOO_SMALL` patterns by product category to tune constraints. If 80% of "Bulk Household" overrides cite fit, add: `product_category = 'Bulk_Household' → storage_type IN [SHELF_LOWER, FLOOR]`.
...
Yes—this trades maintenance convenience for flexibility and transparency. If Merchants A, B, and C have 5, 3, and 8 rules respectively referencing `zone_id = 'ZONE_3'` (16 total), relocating to Zone 5 requires updating all 16 rules. Partial updates risk inconsistency—some products route to Zone 5 while stale rules target non-existent Zone 3.

Rule-level encoding is chosen because it supports nuanced logic mapping tables cannot express. Merchant A might need units → Zone 3 (pick), cases → Zone 5 (reserve), pallets → Zone 6 (VNA). A single merchant-to-zone mapping cannot encode pack-type routing. Rules are also self-documenting—viewing "Merchant A Units" shows `zone_id = 'ZONE_3'` directly, not `LOOKUP(mapping_table)`.

Zone relocations are infrequent (quarterly). Mitigation: Consistent naming ("Merchant_A_Units_Zone3") enables bulk find-replace. Future phases can add bulk edit: "Update all rules where `zone_id = 'ZONE_3'` → 'ZONE_5'". Clone rule templates when onboarding similar merchants.
